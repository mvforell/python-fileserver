...
For user management (adding/deleting users) and manually (using the console of your server) uploading or deleting files please use the provided scripts. (__Note:__ they need to be run from the installed virtualenv, because they use the configuration from the `app` package which in turn imports `flask` etc.) (__Note:__ the scripts use the username `terminal` for log entries, therefore that should best not be used as a real username.)
